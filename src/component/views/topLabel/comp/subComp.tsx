...
import { profile } from "@/component/assests";
...
const SubComp = () => {
  ...
  return (
    <div className="overflow-hidden">
      {userData ? (
        <div
          onClick={() => setSideProfileOpen(true)}
          className="cursor-pointer mr-4 md:mr-0 w-8 h-8 rounded-full flex items-center justify-center text-gray-700 bg-white"
        >
          {userData.photoUrl ? (
            <Image
        ...
        <div className="flex gap-2">
          <Link
            href="/signUp"
            className="hidden md:flex text-white bg-gray-900 px-3 py-1"
          >
            SignUp
          </Link>
          <Link
            href="/signIn"
            className=" text-white border border-purple-800 px-3 py-1 mr-10 md:mr-0"
          >
            SignIn
          </Link>
        </div>
      ...
      <div
        ...
        } duration-500 py-4 px-4 w-72 md:w-80 bg-gray-800 h-full text-gray-100 absolute right-0 top-0 bottom-0 z-50`}
      >
        ...
        {userData && (
          <div className="text-center py-2 text-gray-200">
            ...
            <p className="text-sm">
              <b>Is Email varified :</b>{" "}
              {userData.emailVerified ? "Varified" : "Unvarified"}
            </p>
            ...
            <p className="my-2 text-xs text-red-600 font-light">
              Please check you inbox after Clicking Varify Email
            </p>
            <p className="my-2 text-xs text-red-600 font-light">
              If changes did not reflected please refresh
            </p>
